...
const reOrder = (order) => {
  order.items.forEach(item => {
    store.dispatch('cart/addToCart', { ...item, quantity: item.quantity });
  });
      ...
      <div v-for="order in orders" :key="order.id" class="order-card">
          ...
          <div v-for="item in order.items" :key="item.id" class="product-item">
            ...
            <div class="product-info">
              ...
              <p class="product-meta">
                ...
                Giá: <span class="text-danger fw-bold">{{ item.discount?.toLocaleString('vi-VN') }}₫</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="order-footer">
          <div class="order-summary">
            ...
            <p class="total text-danger fw-bold">{{ order.total.toLocaleString('vi-VN') }}₫</p>
          </div>
          <div class="order-actions">
            <button v-if="order.status === 'Chờ xác nhận' || order.status === 'Đang giao'"
              @click="cancelOrder(order.id)" class="btn btn-cancel">
              <i class="fas fa-times me-1"></i> Hủy đơn
            </button>
            <button v-if="order.status === 'Đã hủy'"
              @click="reOrder(order)" class="btn btn-reorder">
