          ...
          <div v-for="item in wishlistItems" :key="item.id" class="col-md-4 col-lg-3 mb-4">
            <div class="card h-100 shadow-sm border-0 product-card">
              <router-link :to="`/productDetail/${item.id}`" class="text-decoration-none">
                <img v-if="item.image && item.image.length > 0" :src="item.image[0]" class="card-img-top product-img" :alt="item.name">
                <img v-else src="https://via.placeholder.com/300x300.png?text=No+Image" class="card-img-top product-img" :alt="item.name">
              </router-link>
              <div class="card-body text-center d-flex flex-column">
                <h6 class="card-title fw-semibold text-truncate">
                    <router-link :to="`/productDetail/${item.id}`" class="text-dark text-decoration-none stretched-link">
                        {{ item.name }}
                    </router-link>
                </h6>
                <div class="mt-auto">
                    <p class="card-text text-danger fw-bold fs-5 mb-2">{{ formatPrice(item.discount || item.price) }}</p>
                    
                    <div class="position-relative" style="z-index: 2;">
                        <button @click="removeFromWishlist(item.id)" class="btn btn-sm btn-outline-danger">
